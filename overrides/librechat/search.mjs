...
const createSearchAPI = (config) => {
	...
	else if (searchProvider.toLowerCase() === "tavily") return createTavilyAPI(tavilyApiKey, tavilySearchUrl, {
		...tavilySearchOptions,
		httpAgent: httpAgent ?? tavilySearchOptions?.httpAgent,
		httpsAgent: httpsAgent ?? tavilySearchOptions?.httpsAgent
	});
	else if (searchProvider.toLowerCase() === "keenable") return createKeenableAPI(keenableApiKey, keenableApiUrl, {
		...keenableSearchOptions,
		httpAgent: httpAgent ?? keenableSearchOptions?.httpAgent,
		httpsAgent: httpsAgent ?? keenableSearchOptions?.httpsAgent
	});
	else if (searchProvider.toLowerCase() === "crw") return createCrwAPI(crwApiKey, crwApiUrl, {
		...crwSearchOptions,
...
const createSourceProcessor = (config = {}, scraperInstance) => {
	...
	const processResponse = (url, response) => {
		...
		const attribution = getAttribution(url, Object.keys(rawMetadata).length > 0 ? rawMetadata : void 0, logger_);
