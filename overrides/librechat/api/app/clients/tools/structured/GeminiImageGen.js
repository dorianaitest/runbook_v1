...
async function initializeGeminiClient(options = {}) {
  ...
  const proxyBaseUrl = process.env.GEMINI_IMAGE_BASEURL;
  if (proxyBaseUrl) {
    clientConfig.httpOptions = {
      baseUrl: proxyBaseUrl,
      headers: { 'x-litellm-api-key': process.env.GEMINI_IMAGE_API_KEY || '' },
    };
    logger.debug('[GeminiImageGen] Vertex AI via LiteLLM passthrough');
  }
